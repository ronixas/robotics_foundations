...
arm = "left"
pose_point = [0.644, 0.0, 0.8485] # x, y, z
...
def ik_baxter():
    ...
    pt_current, quat_current = query_transformation('base', 'left_gripper')
    point_rot, ori_rot = rotate_pose(pose_point, pose_orientation, np.deg2rad(90), axis="z")
    pose_rot = generate_posemsg(point_rot, ori_rot)
    # Call the Inverse Kinematic service
    limb_joints = ik_service(srv, pose_rot)
    # Move arm
    if limb_joints is not None:
        armcmd.move_to_joint_positions(limb_joints)
